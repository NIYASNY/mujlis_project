...
int total() {
  ...
  for (var i = 0; i < history2.length; i++) {
    a.add(history2[i].IN == 'Income'
        ? parseAmount(history2[i].amount)
        : parseAmount(history2[i].amount) * -1);
  }
...
int income() {
  ...
  List<int> a = [0, 0];
  for (var i = 0; i < history2.length; i++) {
    a.add(history2[i].IN == 'Income' ? parseAmount(history2[i].amount) : 0);
  }
  totals = a.reduce((value, element) => value + element);
  return totals;
}
...
int expenses() {
  ...
  List<int> a = [0, 0];
  for (var i = 0; i < history2.length; i++) {
    a.add(history2[i].IN == 'Income' ? 0 : parseAmount(history2[i].amount) * -1);
  }
  totals = a.reduce((value, element) => value + element);
  return totals;
}
